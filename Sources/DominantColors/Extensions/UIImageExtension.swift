...
extension UIImage {
    ...
    /// - Returns: colors as an array of `UIColor` instances.
    public func dominantColors(max count: Int = 8, options: [DominantColors.Options] = []) throws -> [UIColor] {
        try DominantColors.dominantColors(uiImage: self, maxCount: count, options: options)
    }
